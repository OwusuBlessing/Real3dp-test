...
class GeneFace2Infer:
    def __init__(self, audio2secc_dir, head_model_dir, torso_model_dir, device=None, inp=None):
        ...
        if device == 'cuda':
            ...
            self.mp_face3d_helper = Face3DHelper(use_gpu=False, keypoint_mode='mediapipe')
    ...
    def prepare_batch_from_inp(self, inp):
        ...
        if inp['drv_audio_name'][-4:] in ['.wav', '.mp3']:
            ...
            sample.update({
                'hubert': torch.from_numpy(hubert).float().unsqueeze(0).to(device),
                'f0': torch.from_numpy(f0).float().reshape([1,-1]).to(device),
                'x_mask': x_mask.to(device),
                'y_mask': y_mask.to(device),
                })
            sample['blink'] = torch.zeros([1, t_x, 1]).long().to(device)
            sample['audio'] = sample['hubert']
            sample['eye_amp'] = torch.ones([1, 1]).to(device) * 1.0
            sample['mouth_amp'] = torch.ones([1, 1]).to(device) * inp['mouth_amp']
        ...
        try:
             sample['ref_gt_img'] = load_img_to_normalized_512_bchw_tensor(image_name).cpu()
        ...
        sample['segmap'] = torch.tensor(segmap).float().unsqueeze(0).to(device)
        head_img = self.seg_model._seg_out_img_with_segmap(img, segmap, mode='head')[0]
        sample['ref_head_img'] = ((torch.tensor(head_img) - 127.5)/127.5).float().unsqueeze(0).permute(0, 3, 1,2).to(device) # [b,c,h,w]
        ts.save(sample['ref_head_img'])
        inpaint_torso_img, _, _, _ = inpaint_torso_job(img, segmap)
        sample['ref_torso_img'] = ((torch.tensor(inpaint_torso_img) - 127.5)/127.5).float().unsqueeze(0).permute(0, 3, 1,2).to(device) # [b,c,h,w]
        ...
        sample['bg_img'] = ((torch.tensor(bg_img) - 127.5)/127.5).float().unsqueeze(0).permute(0, 3, 1,2).to(device) # [b,c,h,w]

        # 3DMM, get identity code and camera pose
        coeff_dict = fit_3dmm_for_a_image(image_name, save=False)
        assert coeff_dict is not None
        src_id = torch.tensor(coeff_dict['id']).reshape([1,80]).to(device)
        src_exp = torch.tensor(coeff_dict['exp']).reshape([1,64]).to(device)
        src_euler = torch.tensor(coeff_dict['euler']).reshape([1,3]).to(device)
        src_trans = torch.tensor(coeff_dict['trans']).reshape([1,3]).to(device)
        ...
        if inp['drv_pose_name'] == 'static':
            sample['euler'] = torch.tensor(coeff_dict['euler']).reshape([1,3]).to(device).repeat([t_x//2,1]) # default static pose
            sample['trans'] = torch.tensor(coeff_dict['trans']).reshape([1,3]).to(device).repeat([t_x//2,1])
        else: # from file
            ...
            eulers = convert_to_tensor(drv_pose_coeff_dict['euler']).reshape([-1,3]).to(device)
            trans = convert_to_tensor(drv_pose_coeff_dict['trans']).reshape([-1,3]).to(device)
            len_pose = len(eulers)
        ...
        if inp.get("map_to_init_pose", 'True') in ['True', True]:
            diff_euler = torch.tensor(coeff_dict['euler']).reshape([1,3]).to(device) - sample['euler'][0:1]
            sample['euler'] = sample['euler'] + diff_euler
            diff_trans = torch.tensor(coeff_dict['trans']).reshape([1,3]).to(device) - sample['trans'][0:1]
        ...
        camera = torch.tensor(camera).to(device).float()
    ...
    def forward_audio2secc(self, batch, inp=None):
        if inp['drv_audio_name'][-4:] in ['.wav', '.mp3']:
            ...
            batch['exp'] = torch.FloatTensor(drv_motion_coeff_dict['exp']).to(device)
    ...
    def get_driving_motion(self, id, exp, euler, trans, batch, inp):
        ...
        batch['drv_secc'] = drv_secc_colors.to(device)
        batch['src_secc'] = src_secc_color.to(device)
        batch['cano_secc'] = cano_secc_color.to(device)
        ...
        if inp['low_memory_usage']:
            ...
            with torch.no_grad():
                for i in tqdm.trange(num_frames, desc="Real3D-Portrait is rendering frames"):
                    ...
                    cond={'cond_cano': cano_secc_color,'cond_src': src_secc_color, 'cond_tgt': drv_secc_colors[i:i+1].to(device),
...
if __name__ == '__main__':
    ...
    parser.add_argument("--out_name", default='result.mp4') # output filename
